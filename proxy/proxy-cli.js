...
// node_modules/@gfld/compositor-proxy-cli/src/main.ts
var import_compositor_proxy2 = require("@gfld/compositor-proxy");
var import_node_http = require("node:http");
var import_node_child_process = require("node:child_process");

// node_modules/@gfld/compositor-proxy-cli/src/main-controller.ts
var import_compositor_proxy = require("@gfld/compositor-proxy");

// node_modules/@gfld/compositor-proxy-cli/src/main-args.ts
var import_node_util = require("node:util");
var import_node_fs = require("node:fs");
var import_ajv = __toESM(require_ajv());
var import_ajv_formats = __toESM(require_dist());

// node_modules/@gfld/compositor-proxy-cli/src/app-config-schema.json
var app_config_schema_default = {
...
var validate = ajv.compile(app_config_schema_default);
var options = {
  help: {
    type: "boolean",
    default: false,
...
function parseEncoderArg(rawArg) {
  switch (rawArg) {
    case "x264":
    case "nvh264":
    case "vaapih264":
...
function parseApplicationsArg(rawArg) {
  if (rawArg === "") {
    return {
      "/gtk4-demo": {
        name: "GTK Demo",
  ...
  const applicationMappingFileContents = (0, import_node_fs.readFileSync)(rawArg, "utf8");
...
var args = Object.fromEntries(
  Object.entries((0, import_node_util.parseArgs)(parseArgsConfig).values).map(([key, value]) => {
    switch (key) {
      case "encoder":
      case "applications":
...
// node_modules/@gfld/compositor-proxy-cli/src/main-controller.ts
var allowHeaders = "Content-Type, X-Compositor-Session-Id, Authorization, WWW-Authenticate";
var maxAge = "36000";
var messageSerial = 0;
var logger = (0, import_compositor_proxy.createLogger)("main");
var basicAuth = args["basic-auth"];
var user;
var password;
if (basicAuth) {
  ;
...
function authRequest(request, response, _url) {
  if (user && password) {
    const authHeader = request.headers["authorization"];
    if (authHeader === void 0) {
      response.writeHead(401, "Not authenticated", {
...
function isToMainProcessMessage(message) {
  return message.type === "launchAppSuccess" || message.type === "launchAppFailed";
}
function sendMessageWithReply(childProcess, message, timeout = 1e4) {
  return new Promise((resolve, reject) => {
    ...
    const replyListener = (message2) => {
      if (isToMainProcessMessage(message2)) {
        if (message2.payload.replySerial === sendSerial) {
...
async function handleGET(childProcess, compositorSessionId, config, request, response, url, applications) {
  let appName;
  let appExecutable;
  let appArgs;
  let appEnv;
  for (const [path2, { name, executable, args: args2, env }] of Object.entries(applications)) {
...
process.on("uncaughtException", (e) => {
  logger2.error("	name: " + e.name + " message: " + e.message);
  logger2.error("error object stack: ");
  logger2.error(e.stack ?? "");
});
var logger2 = (0, import_compositor_proxy2.createLogger)("main");
var sessionProcesses = {};
function main() {
  logger2.info(`Starting compositor proxy with args: ${(0, import_node_util2.inspect)(args)}`);
  const config = {
    server: {
      http: {
  ...
  const server = (0, import_node_http.createServer)({ noDelay: true });
  server.on("upgrade", (request, socket) => {
    ...
    childProcess.send(wsUpgrade, socket);
  });
  server.on("request", (request, response) => {
    ...
    if (request.method === "GET") {
      ...
      if (childProcess === void 0) {
        logger2.info("No proxy session exists for this compositor, spawning a new one.");
        childProcess = (0, import_node_child_process.fork)(import_node_path.default.join(__dirname, "./SessionProcess"));
        childProcess.once("exit", (code, signal) => {
          logger2.info(`Proxy session exited: ${signal || code}`);
          delete sessionProcesses[compositorSessionId];
        });
        childProcess.once("error", (err) => {
          logger2.error(`Proxy session error: ${err.message}`);
          delete sessionProcesses[compositorSessionId];
        });
  ...
  server.on("listening", () => {
    process.on("SIGTERM", () => {
      logger2.info("Received SIGTERM. Closing connections.");
      server.closeAllConnections();
      logger2.info("All Connections closed. Goodbye.");
      process.exit();
    });
    logger2.info(`Compositor proxy started. Listening on ${host}:${port}`);
  });
  server.on("error", (err) => {
